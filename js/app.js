const APP = {
  ...
  openDatabase: (nextStep) => {
    ...
    dbOpenRequestSearch.onsuccess = (ev) => {
      ...
      let dbOpenRequestSuggest = indexedDB.open(
        "suggestStore",
        APP.suggestVersion
      );
      dbOpenRequestSuggest.onupgradeneeded = (ev) => {
        ...
        let suggestStore = APP.DBsuggest.createObjectStore(
          "suggestStore",
          options
        );
      };
  ...
  checkDB: (storeName, keyValue, title) => {
    ...
    tx.oncomplete = () => {
      if (title) {
        //title will only be there if we are dealing with Suggest page
    ...
    getResult.onsuccess = (ev) => {
      if (getResult.result) {
        //getResult.result returns undefined if no match found
  ...
  getSearchResults: (keyword) => {
    ...
    if (APP.results.length === 0) {
      ...
      if (APP.isOnline) {
        //if no results in DB and online, do a new fetch
        APP.objSearch.keyword = keyword;
        let url = new URL(
          `${APP.tmdbBASEURL}search/movie${APP.tmdbAPIKEY}&query=${keyword}`
        );
        APP.getData(url);
      } else {
      ...
      // if there are results, just display
      APP.navigate();
    }
  },
  getSuggestedResults: (movieId, title) => {
    ...
    if (APP.results.length < 1) {
      ...
      if (APP.isOnline) {
        ...
        let url = new URL(
          `${APP.tmdbBASEURL}/movie/${movieId}/recommendations${APP.tmdbAPIKEY}`
        );
        APP.getData(url);
      } else {
        APP.nextPage = `/404page.html`;
    ...
    if (APP.results.length < 1) {
      ...
      APP.results.forEach((movie) => {
        ...
        if (movie.release_date) {
          //for upcoming movies, release date property doesn't exist
          releaseDate.textContent = `Released: ${movie.release_date.substring(
            0,
            4
          )}`;
        } else {
  ...
  navigate: () => {
    ...
    window.location = window.location.origin + APP.nextPage;
  },
};
